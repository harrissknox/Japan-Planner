...
let db;
// When saving data to Firestore, we set pendingSaves > 0 so that
// the snapshot listener knows to ignore stale snapshots that might
// arrive before the save completes.  Each save increments
// pendingSaves; the snapshot handler decrements it when skipping.
let pendingSaves = 0;
try {
...
function saveToFirestore() {
  if (!db) return;
  const payload = { couples };
  // Increment pendingSaves so we can ignore the next snapshot(s) that
  // arrive before our changes are committed.  Without this, stale
  // snapshots can overwrite local modifications if they arrive out
  // of order.
  pendingSaves++;
  db.collection('planner').doc('main').set(payload).catch(err => {
...
function initRealtimeUpdates() {
  ...
  db.collection('planner').doc('main').onSnapshot(doc => {
    // If there are pending saves, skip this snapshot.  This prevents
    // outdated Firestore data from overwriting recent local changes.
    if (pendingSaves > 0) {
      pendingSaves--;
      return;
    }
    if (doc.exists) {
      ...
      if (data && Array.isArray(data.couples)) {
        // Merge Firestore data into the local couples array.  We
        // update fields on existing objects rather than replacing
        // them wholesale to preserve references used elsewhere.
...
function deleteEventById(eventId) {
  ...
  // Persist changes both locally and remotely and re-render after deletion
  saveData();
  saveToFirestore();
  render();
}
...
function renderEventForm(couple) {
  ...
  form.addEventListener('submit', e => {
    ...
    costInput.disabled = false;
    // Persist the updated events list locally and remotely and re-render
    saveData();
    saveToFirestore();
    render();
  });
